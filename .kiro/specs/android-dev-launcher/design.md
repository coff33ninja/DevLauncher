...
The Android Developer Launcher is a **plugin-based** home screen replacement designed for developers. It provides a minimal, fast core with a powerful global search interface, and extends functionality through a modular plugin system.

### Core Philosophy

**"The launcher does almost nothing… but enables everything."**

The core launcher is intentionally minimal:
- App launcher (multi-screen grid)
- Global search bar (the primary interaction point)
- Plugin manager
- Event bus for plugin communication

Everything else—terminal, AI assistant, API tester, remote desktop—is a **plugin**.

### Key Architectural Decisions

1. **Kotlin-Only**: No Python backend. Pure Kotlin/Android for simplicity and performance.
2. **BYO Gemini**: Users provide their own Gemini API key (optional). No built-in key management complexity.
3. **Plugin-First**: All features beyond core launcher are plugins. Even "built-in" features are implemented as plugins.
4. **Command-Driven**: Global search acts as a command palette (like Raycast/Spotlight) where plugins register commands.
5. **Lifecycle-Aware**: Aggressive lifecycle management for Android constraints (battery, memory, background limits).

### What This Is NOT

- ❌ Not a full DevOS with everything built-in
- ❌ Not a Python + Kotlin hybrid system
- ❌ Not a widget-heavy traditional launcher
- ❌ Not an enterprise-scale system with round-robin key management

### What This IS

- ✅ A fast, keyboard-driven launcher platform
- ✅ A plugin ecosystem for developer tools
- ✅ A Raycast/Alfred-style command interface for Android
- ✅ A foundation that can grow without bloating the core

## Architecture

The system follows a **plugin-based architecture** with a minimal core and extensible plugin system.

```mermaid
graph TD
    A[Launcher Core] --> B[App Launcher]
    A --> C[Global Search]
    A --> D[Plugin Manager]
    A --> E[Event Bus]
    ...
    K[Plugins] --> L[Terminal Plugin]
    K --> M[AI Assistant Plugin]
    K --> N[API Tester Plugin]
    K --> O[Remote Desktop Plugin]
    K --> P[Docker Plugin]
    K --> Q[... More Plugins]
    ...
    B --> R[Android PackageManager]
    A --> S[Settings/DataStore]
```

### Core Layers

1. **Launcher Core** (Sacred - DO NOT BLOAT)
   - App launcher (multi-screen grid)
   - Global search bar
   - Plugin manager
   - Event bus
...
2. **Plugin System** (The Real Product)
   - Plugin loader
   - Plugin registry
   - Command registry
...
The absolute minimum needed for a functional launcher.

**Components**:
- **HomeScreenPager**: Multi-screen app grid with swipe navigation
- **AppRepository**: Access to installed apps via PackageManager
- **GlobalSearchBar**: Primary interaction point - command palette style
- **PluginManager**: Loads, manages, and coordinates plugins
- **EventBus**: Decoupled communication between plugins
...
- Persist minimal settings (theme, grid size)

**What It Does NOT Do**:
- ❌ AI integration (that's a plugin)
- ❌ Terminal emulation (that's a plugin)
- ❌ Network tools (those are plugins)
- ❌ Complex background services (plugins handle their own)

### 2. Plugin System

**Technology**: Kotlin, Reflection/ServiceLoader

The plugin system is the heart of extensibility.

#### Plugin Interface

```kotlin
interface Plugin {
    val id: String              // Unique identifier (e.g., "terminal")
    val name: String            // Display name (e.g., "Terminal")
    val version: String         // Semantic version (e.g., "1.0.0")
    val description: String     // Short description
    val permissions: PluginPermissions  // Required permissions
...
**UI Plugin** - Renders UI components:
```kotlin
interface UIPlugin : Plugin {
    fun render(): @Composable () -> Unit
    fun getScreenConfig(): ScreenConfig?  // Optional dedicated screen
}
...
**Command Plugin** - Registers commands for global search:
```kotlin
...
data class Command(
    val name: String,              // "ssh", "ask", "run"
    val description: String,       // "Connect to SSH server"
    val aliases: List<String>,     // ["connect", "remote"]
...
sealed class CommandResult {
    ...
    data class OpenUI(val composable: @Composable () -> Unit) : CommandResult()
}
```
...
interface BackgroundPlugin : Plugin {
    ...
    fun getWorkRequest(): PeriodicWorkRequest?  // Optional WorkManager job
}
```

#### Plugin Context

The context provides plugins access to core functionality:
...
class PluginStorage(private val pluginId: String) {
...
Plugins communicate via events without tight coupling:
...
class PluginLoader(private val context: Context) {
    ...
    fun loadBuiltInPlugins() {
        // Load plugins compiled into the app
    ...
    fun loadPlugin(plugin: Plugin) {
        ...
        if (!validatePermissions(plugin.permissions)) {
            throw SecurityException("Plugin ${plugin.id} requires unapproved permissions")
        }
        ...
        val pluginContext = PluginContext(
            appContext = context,
            eventBus = eventBus,
            storage = PluginStorage(plugin.id),
...
The global search is the primary interaction point - think Raycast/Spotlight.

**Features**:
- **Command Execution**: Type commands like `ssh home-server`, `ask explain docker`, `run container`
- **App Search**: Fuzzy search for installed apps
- **Plugin Commands**: All plugins register their commands here
- **Ranked Results**: Exact match → fuzzy match → AI suggestions
...
class GlobalSearch(
    ...
    suspend fun search(query: String): List<SearchResult> {
        ...
        commandRegistry.findCommand(query)?.let { command ->
            results.add(SearchResult.Command(command, priority = 100))
        }
        
        // 2. Fuzzy match apps
        val matchingApps = appRepository.search(query)
        results.addAll(matchingApps.map { SearchResult.App(it, priority = 50) })
        
        // 3. Fuzzy match commands
        ...
        // 4. AI suggestions (if AI plugin enabled)
        eventBus.publish("search.query", SearchQueryEvent(query))
        // AI plugin can respond with suggestions via event
...
sealed class SearchResult(val priority: Int) {
    ...
    data class Command(val command: Command, override val priority: Int) : SearchResult(priority)
    data class AISuggestion(val suggestion: String, override val priority: Int) : SearchResult(priority)
}
```
...
Multiple home screens for organization, but simpler than the original design.

**Screens**:
1. **Main** - Favorite apps + global search
2. **Development** - Dev tools (if dev plugins installed)
3. **Communication** - Messaging apps
4. **Productivity** - Notes, calendar, tasks
5. **Custom** - User-defined

**Key Difference from Original**:
- Screens are just **containers for app shortcuts**
- Plugins can request a dedicated screen, but it's optional
- No complex widget system - plugins render their own UI when invoked

### 5. Settings & Storage

**Technology**: Kotlin, Jetpack DataStore

Minimal settings for core + plugin-specific storage.

**Core Settings**:
- Theme (Light/Dark/System)
- Grid size (3x4, 4x5, 5x6)
- Enabled plugins
- Plugin permissions

**Plugin Storage**:
- Each plugin gets isolated storage via `PluginStorage`
- Plugins manage their own settings
- No shared state between plugins (use EventBus for communication)
